#!/bin/bash
# Подключиться по SSH к on-demand GPU (Tesla T4) инстансу в зоне europe-west4-a.
#
...
# Доп. аргументы пробрасываются в gcloud (например: ./a_ssh.sh --command='nvidia-smi').
...
if ! g compute instances describe "$INSTANCE_NAME" --zone="$TARGET_ZONE" \
    ...
    if [ -z "$INST_ZONE" ]; then
        echo "ℹ️  Инстанса '$INSTANCE_NAME' нет ни в одной зоне — подними: ./a_start.sh"
    else
