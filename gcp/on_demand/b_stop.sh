#!/bin/bash
# Остановить on-demand GPU (Tesla T4) инстанс в зоне europe-west4-b.
#
# Парный к b_start.sh: гасит инстанс в своей зоне (boot-диск остаётся там же,
# платишь только за storage). on-demand не вытесняется — stop это обычный
# способ «выключить на ночь». Поднять обратно — b_start.sh.
#
