#!/bin/bash
# Остановить on-demand GPU (Tesla T4) инстанс в зоне europe-west4-a.
#
# Парный к a_start.sh: гасит инстанс в своей зоне (boot-диск остаётся там же,
# платишь только за storage). on-demand не вытесняется — stop это обычный
# способ «выключить на ночь». Поднять обратно — a_start.sh.
#
