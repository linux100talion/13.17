#!/bin/bash
# Снять снапшот boot-диска dev-workspace-1317 в зоне europe-west4-b.
#
...
# Тег даты переопределяется:  TAG=manual ./b_take.sh
#
...
TARGET_ZONE="europe-west4-b"     # ← единственное, чем отличаются a/b/c_take.sh
TAG="${TAG:-$(date +%Y%m%d)}"
REGION="${TARGET_ZONE%-*}"               # europe-west4-b → europe-west4
