#!/bin/bash
# Поймать SPOT GPU (Tesla T4) инстанс в зоне europe-west4-b.
#
...
#
# Тогглы (env):
#   MACHINE_TYPE=n1-standard-4 ./11_spot_gpu_b.sh  # меньше host'ов → реже EXHAUSTED
#   SPOT=0 ./11_spot_gpu_b.sh                       # on-demand вместо spot (дороже, стабильнее)
#   REFRESH=1 ./11_spot_gpu_b.sh                    # перезалить устаревший диск целевой зоны из свежего снапшота
#
...
TARGET_ZONE="europe-west4-b"     # ← единственное, чем отличаются 10–12_spot_gpu_{a,b,c}.sh
