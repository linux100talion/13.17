...
#
# Тогглы (env):
#   MACHINE_TYPE=n1-standard-4 ./a_start.sh  # меньше host'ов → реже EXHAUSTED
#   SPOT=0 ./a_start.sh                       # on-demand вместо spot (дороже, стабильнее)
#   REFRESH=1 ./a_start.sh                    # перезалить устаревший диск целевой зоны из свежего снапшота
#
...
TARGET_ZONE="europe-west4-a"     # ← единственное, чем отличаются {a,b,c}_start.sh
