#!/bin/bash
# Остановить SPOT GPU (Tesla T4) инстанс в зоне europe-west4-a.
#
# Парный к a_start.sh: гасит инстанс в СВОЕЙ зоне (boot-диск зонален и остаётся
...
# тем же a_start.sh (или ../02_power_manager.sh start с ZONE=europe-west4-a).
#
...
TARGET_ZONE="europe-west4-a"     # ← единственное, чем отличаются a/b/c_stop.sh
...
if g compute instances stop "$INSTANCE_NAME" --zone="$TARGET_ZONE"; then
    ...
    echo "   Поднять обратно: ./a_start.sh"
else
