...
#
# Тогглы (env):
#   MACHINE_TYPE=n1-standard-4 ./b_start.sh  # меньше host'ов → реже EXHAUSTED
#   SPOT=0 ./b_start.sh                       # on-demand вместо spot (дороже, стабильнее)
#   REFRESH=1 ./b_start.sh                    # перезалить устаревший диск целевой зоны из свежего снапшота
#
...
TARGET_ZONE="europe-west4-b"     # ← единственное, чем отличаются {a,b,c}_start.sh
