#!/bin/bash
# Конвертировать инстанс dev-workspace-1317 в зоне europe-west4-b из on-demand
# (STANDARD) в SPOT БЕЗ пересоздания — `gcloud instances set-scheduling`.
#
# Обратное к ../on_demand/b_convert.sh. In-place: инстанс и его boot-диск
...
TARGET_ZONE="europe-west4-b"     # ← единственное, чем отличаются a/b/c_convert.sh
...
if ! g compute instances describe "$INSTANCE_NAME" --zone="$TARGET_ZONE" \
    ...
    if [ -z "$INST_ZONE" ]; then
        ...
        echo "   Создать SPOT на существующем диске: SPOT=1 ../08_add_gpu.sh (или ./b_start.sh)."
    else
