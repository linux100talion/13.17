...
(
    ...
    sleep 5
    for i in 1 2 3; do
        echo "[Попытка $i] Запрашиваем HIGHRES_IMU (ID 105), RAW_IMU (ID 27), ATTITUDE_QUATERNION (ID 31)..." >> /tmp/mavros_stream.log
        ...
        # ATTITUDE_QUATERNION на 200 Гц — из него MAVROS собирает /mavros/imu/data (ориентация
        # полётника); без запроса топик молчит до первого арма (auto_bag.sh запрашивает на арме)
        ros2 service call /mavros/set_message_interval mavros_msgs/srv/MessageInterval "{message_id: 31, message_rate: 200.0}" >> /tmp/mavros_stream.log 2>&1
