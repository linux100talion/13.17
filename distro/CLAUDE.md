...
index»), и `ros2 bag record` из auto-bag не мог стартовать. Чинить при деплое: гасить ноды
изнутри контейнера (`docker exec $CONTAINER pkill …`) или `docker exec` с отдельным
процессом-супервизором. Там же исправлено (обе службы): `pkill` наблюдателя /mavros/state —
только в своей сессии (`-s`), а смерть наблюдателя = `exit 1` (иначе служба гасла с кодом 0).
