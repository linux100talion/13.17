#!/usr/bin/env bash
# cmd/3/3.sh — A/B-полёт кандидата DpVins BRAKE 5 (кап 2.0, ki 15) под порывами (серия dphold_vs_dpvins).
# Профили: dphold/baseline + dpvins/brake5 (кандидат) + vins/baseline + loiter/baseline,
...
. src/control/profiles/dpvins/brake5.txt
...
echo ">>> cmd/3/3.sh: профили dphold/baseline + dpvins/brake5 + vins/baseline + loiter/baseline;" \
     "BS_DPVINS_KI=$BS_DPVINS_KI BRAKE=$BS_DPVINS_POS_BRAKE/$BS_DPVINS_POS_BRAKE_VMAX WIND_SPD=$WIND_SPD WIND_GUST=\"$WIND_GUST\""
