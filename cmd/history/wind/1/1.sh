#!/usr/bin/env bash
# cmd/1/1.sh — A/B-полёт кандидата DpVins ki 30 под порывами (серия dphold_vs_dpvins).
# Профили: dphold/baseline + dpvins/ki30 (кандидат) + vins/baseline + loiter/baseline,
...
export PROFILES="dphold/baseline dpvins/ki30 vins/baseline loiter/baseline vinshold/baseline mission/baseline legacy/baseline world/wind1_gust8"
