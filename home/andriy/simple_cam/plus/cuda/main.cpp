...
int main() {
    ...
    // === 1. ЭТО ПИШЕМ ДО ЦИКЛА ===
    // Жестко резервируем кусок видеопамяти под размер кадра
    cv::cuda::GpuMat gpu_raw(HEIGHT, WIDTH, CV_8UC1);
    
    // Резервируем память под результаты, чтобы не аллоцировать ее каждый кадр
    cv::cuda::GpuMat gpu_color;
    cv::cuda::GpuMat gpu_mono;
    ...
    while (true) {
        ...
        try {
            ...
            // Гамма на CPU (пока оставим так)
            cv::Mat img8_gamma;
            cv::LUT(img8, gamma_table, img8_gamma);

            // Чтение параметров с веб-тюнера
            float c_gain = tune_gain;
            float c_r = tune_r;
            float c_g = tune_g;
            float c_b = tune_b;

            // === 2. ЭТО ВНУТРИ ЦИКЛА ЗАХВАТА ===
            
            // Загружаем данные в уже зарезервированную видеопамять (H2D)
            gpu_raw.upload(img8_gamma);

            // Дебайер на GPU
            // Обрати внимание: в C++ используется cv::cuda::demosaicing
            cv::cuda::demosaicing(gpu_raw, gpu_color, cv::COLOR_BayerGR2BGR);

            // Баланс белого и Gain прямо в видеопамяти
            if (c_gain != 1.0f || c_r != 1.0f || c_g != 1.0f || c_b != 1.0f) {
                cv::cuda::split(gpu_color, gpu_channels);
                
                // В C++ для умножения GpuMat на константу намного быстрее 
                // использовать convertTo с параметром alpha (масштаб), чем cuda::multiply
                gpu_channels[0].convertTo(gpu_channels[0], -1, c_b * c_gain); // Blue
                gpu_channels[1].convertTo(gpu_channels[1], -1, c_g * c_gain); // Green
                gpu_channels[2].convertTo(gpu_channels[2], -1, c_r * c_gain); // Red
                
                cv::cuda::merge(gpu_channels, gpu_color);
            }

            // Генерируем Ч/Б для трекинга 
            cv::cuda::cvtColor(gpu_color, gpu_mono, cv::COLOR_BGR2GRAY);

            // Выгружаем готовые кадры в обычную RAM (D2H)
            ...
            // 5. СЖАТИЕ И ОТПРАВКА (каждый 2-й кадр)
            frame_counter++;
