...
def capture_loop():
    ...
    while True:
        ...
        # Переводим в цвет для стрима (замени на COLOR_BayerBG2BGR если цвета искажены)
        # img_color = cv2.cvtColor(img8, cv2.COLOR_BayerRG2BGR) 
        # Переводим в цвет для стрима
        img_color = cv2.cvtColor(img8, cv2.COLOR_BayerGR2BGR)

        # Уменьшаем кадр в 2 раза. Стрим 1920x1200 по HTTP будет сильно лагать, 
        # 960x600 вполне достаточно для превью на ноуте.
