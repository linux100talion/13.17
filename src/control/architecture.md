...
`src/lab/alt_hold_bootstrap.py` (~865 строк) в переиспользуемое лётное ядро управления.
...
**Северная звезда (лётное назначение):** до инициализации VINS дать пилоту НАШ простой
...
- **`BS_STAB`** → `recipes.build_stabilizers(cfg, spec)` — стабилизатор ПО ИМЕНИ, с
  ...
  `DpRollHold`, лётный пре-VINS = `DpRollHold+DpYawHold`.
...
- **микс**: «оператор + только yaw» = `[DpYawHold]`; лётный пре-VINS = `[DpRollHold,DpYawHold]`
...
**Трёхпозиционник CH6 = селектор стабилизации** (токен миссии `pilot[t]`, только
...
дёргает высоту (реальный случай: пилот держал газ на −1.0, считая себя в MANUAL).
Две точки врезки, разный момент взвода: `Arbiter` (взвод на каждом щелчке в
MANUAL; до открытия газ = центр) и `Control`-шаг миссии (`pilot_thr=True` только
при живом пилоте joy/ros — scripted-эталоны не меняются). В assisted газ теперь
РАБОТАЕТ: стик вне центра = вертикальная скорость (ALT_HOLD), отпустил — контур
AltHold перецеливается на ТЕКУЩУЮ высоту (не тянет назад на старую уставку).
Осталось: MANUAL-seize в воздухе не испытан, доставка намерения на реальный Orin
мимо FCU (открытая ветка — laptop_move.md §3).
...
5. Порт на реальный Orin (colcon в orin-контейнере; VinsHold уже на VINS-позе, не gt).
