...
- частичные стабилизаторы (FlowDamper roll + YawHold yaw) → эти оси регулируются,
...
# --- 2. Частичные: FlowDamper(roll) + YawHold(yaw), pitch НЕ занят → пилот ---
stack = ControlStack([FlowDamper(), YawHold()], RcTransmitter(), NoExcitation())
s = state(pilot_roll=1400, pilot_pitch=1580, pilot_yaw=1450,
...
check("частичные: roll занят FlowDamper (≠ стик 1400)", rc.roll != 1400 and rc.roll == 1540)
check("частичные: yaw занят YawHold (≠ стик 1450)", rc.yaw != 1450 and rc.yaw == 1518)

# --- 3. Одиночный стабилизатор как список из одного (обратная совместимость) ---
stack = ControlStack([FlowDamper()], StaticSetpoint(), NoExcitation())
s = state(pilot_roll=1400, pilot_pitch=1580, pilot_yaw=1450,
