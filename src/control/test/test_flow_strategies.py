...
- FlowDamper (roll): демпф к цели, velocity-assist (стик=цель → 0 коррекции),
  ПОКАДРОВАЯ интеграция (тот же flow_seq не двигает PID), stale-fade, conf-blend.
- YawHold (yaw): чистый rate-демпф (ki=0), velocity-assist.
...
from control_pkg.domain.control.stabilization import FlowDamper, YawHold  # noqa: E402
...
check("FlowDamper stale → fade в центр", rc.roll == 1500)

# confidence ниже порога → авторитет 0
fd4 = FlowDamper()
fd4.enter(st(-1))
rc = fd4.update(st(1, lat=5.0, conf=0.0, now=0.05), Setpoint(), 0.05)
check("FlowDamper conf<min → авторитет 0 (центр)", rc.roll == 1500)

# --- YawHold: чистый rate-демпф (ki=0) ---
yh = YawHold()  # kp6 ki0 → yu=6*err
yh.enter(st(-1))
rc = yh.update(st(1, yaw=3.0, now=0.05), Setpoint(), 0.05)
check("YawHold rate-демпф (yaw=1518)", rc.yaw == 1518)
# velocity-assist: стик=цель → ошибка 0
yh2 = YawHold(cmd_gain=1.0)
yh2.enter(st(-1))
rc = yh2.update(st(1, yaw=3.0, now=0.05), Setpoint(c_yaw=3.0), 0.05)
check("YawHold velocity-assist: стик=поток → yaw=центр", rc.yaw == 1500)
# YawHold трогает ТОЛЬКО yaw (roll/pitch = центр в его выходе)
check("YawHold владеет только yaw", rc.roll == 1500 and rc.pitch == 1500)
