...
# (_dot), а не разностью ошибки. Прежняя разность несла шаг утечки −err·fdt/T
# («0.6% за кадр» — но это при err≈0; на намотанной ошибке 350–430° тот же член
...
# --- ПРУЖИНА (регрессия разбора yaw_ab_ki60_win03/spring, 2026-08-27) ---
# Лётный сценарий: kp=0, борт не успевает за уставкой → ошибка НАМАТЫВАЕТСЯ (в полёте
# до 350–430°). Старый D-член (разность ошибки) после отпускания стика сливал её через
# утечку в PWM и разматывал борт обратно на 92–96% разворота. Требование: при kp=0
# накопленная ошибка ИНЕРТНА — нет вращения (flow_yaw=0) и нет команды (стик отпущен)
# → нет и выхода, сколько бы ни намотало.
spr = _yh(cmd_gain=9.28, leak_sec=8.0)          # kp0 ki0 kd6 — штатный freefly-профиль
spr.enter(st(-1))
for k in range(1, 61):                          # 3 с полного стика, борт СТОИТ (yaw=0)
...
off.enter(st(-1))
off.update(st(1, yaw=0.0, now=0.05), Setpoint(c_yaw=0.3), 0.05)
