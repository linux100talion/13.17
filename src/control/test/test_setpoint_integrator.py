...
# ⚠️ ЗНАК СТИКА ОТРИЦАТЕЛЬНЫЙ, и это не описка. `flow_lateral>0` = борт идёт ВЛЕВО
# (мир в кадре уезжает вправо), значит «не возражать против этого сноса» — стик ВЛЕВО,
# то есть c_right<0; в цель это переводит минус в `DpRollHold._cmd` (замер R2: без него
# ось отрабатывала боковую команду ЗЕРКАЛЬНО — цель +2.98 уводила борт влево на 4.4 м).
# Тест писался ДО той правки и держал +0.5 против +5 — то есть проверял реализацию,
# а не физику, и потому пережил зеркало. Парный тест в test_flow_strategies.py починен
