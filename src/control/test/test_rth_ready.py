...
# --- 10ж. ЗРЕЛОСТЬ ЗАЩЁЛКИВАЕТСЯ. Полёт 171337: через 2 с после READY борт пошёл
# в разворот, Δyaw дрогнул → зрелость снялась → /vins/bridge_ok=False → мост
# закрыт → «закрытый мост после латча» → LOST терминально. Петля из трёх звеньев,
# заведённая одним чихом. Теперь доказанная зрелость не снимается ничем, кроме
# перерождения VINS; за здоровьем потока следит BridgeGate ---
r = mk(dyaw_tol=2.0)
st, t, seq = run(r, 2000.0, 9.0, dyaw=0.0)
check("зрелость набрана, дом залатчен", r.ripe and r.state == 'ready')
st, t, seq = run(r, t, 0.2, seq0=seq, dyaw=30.0)      # рывок Δyaw на 30°
check("рывок Δyaw после латча: зрелость НЕ снимается", r.ripe)
check("и возврат не потерян", r.state == 'ready')
st, t, seq = run(r, t, 1.0, seq0=seq, odom=0)         # поток обмелел
check("даже нездоровый поток не снимает доказанную зрелость", r.ripe)
# перерождение VINS — рама новая, доказывать заново
st, t, seq = run(r, t, 0.2, seq0=seq, reb=1)
check("перерождение VINS: зрелость СНЯТА (рама новая)", not r.ripe)
st, t, seq = run(r, t, 9.0, seq0=seq, reb=1, dyaw=0.0)
check("после перерождения зрелость набирается заново", r.ripe)

# --- 10з. ЧЕК Δyaw МОЛЧИТ НА ВИРАЖЕ (dyaw_wz): курс AHRS и курс VINS идут разными
# трактами с разной задержкой, на развороте их разность гуляет ЛАГОМ, а не уходом
# (замер 171337: размах 5.1° при 41.6 °/с и 2.4° при 9.5 °/с в одном полёте) ---
import math as _m
r = mk(dyaw_tol=2.0, dyaw_wz=15.0)
t, seq, yaw = 2100.0, 0, 0.0
for i in range(400):                       # 20 с разворота 40 °/с, Δyaw пляшет ±5°
    t += 0.05
    seq += 1
    yaw += _m.radians(40.0) * 0.05
    r.update(snap(t, seq=seq, yaw=yaw, dyaw=5.0 * (1 if i % 2 else -1)), sane=True)
check("вираж 40 °/с: пляска Δyaw не мешает зрелости", r.ripe)
# на медленном развороте чек работает как прежде
r2 = mk(dyaw_tol=2.0, dyaw_wz=15.0)
t, seq, yaw = 2200.0, 0, 0.0
for i in range(400):
    t += 0.05
    seq += 1
    yaw += _m.radians(5.0) * 0.05          # 5 °/с — ниже гейта
    r2.update(snap(t, seq=seq, yaw=yaw, dyaw=5.0 * (1 if i % 2 else -1)), sane=True)
check("5 °/с: чек работает, пляска Δyaw держит зрелость закрытой", not r2.ripe)
# гейт выключен (dyaw_wz=0) — поведение до 2026-09-09
r3 = mk(dyaw_tol=2.0, dyaw_wz=0.0)
t, seq, yaw = 2300.0, 0, 0.0
for i in range(400):
    t += 0.05
    seq += 1
    yaw += _m.radians(40.0) * 0.05
    r3.update(snap(t, seq=seq, yaw=yaw, dyaw=5.0 * (1 if i % 2 else -1)), sane=True)
check("dyaw_wz=0: гейт выключен, вираж снова блокирует зрелость", not r3.ripe)
