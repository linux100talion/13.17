...
Зрительная часть лётного пре-VINS демпфера. Подписывается на кадр и ω, гоняет
...
class RosPerception:
    def __init__(self, node, cam_w, cam_h, R_cam_imu, rotflow_sign=1.0,
        ...
        # Плюс это совпадает с реальным бортом: на Orin IMU приходит с полётника через
        ...
        if ipm_wz_bias_max is not None:
            extra['ipm_wz_bias_max'] = float(ipm_wz_bias_max)
        ...
        # профиль LV=2/реальный борт; вертикаль EKF переживает смерть VINS
