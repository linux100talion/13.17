...
class RosTelemetry:
    ...
    def _on_bridge(self, m):
        ...
        #  [Δyaw якоря, град] [Δyaw сейчас, град]' — шестое и седьмое поля
        # с 2026-09-09, старые записи без них (шестое — снимок шва, седьмое —
        # мгновенная разность «курс AHRS − курс VINS», по её устойчивости
        # судится зрелость; сборка — ray_tracer, формат заперт
        # src/nav/test/test_bridge_gate.py §9)
        w = m.data.split()
        if len(w) >= 5:
