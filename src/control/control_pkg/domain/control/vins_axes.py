...
class DpVins(StabilizationStrategy):
    ...
    def _trim_armed(self, v) -> None:
        if self.wind is not None:
            self.wind.mark_learned(who=self, value=v)
        else:
    ...
    def update(self, s: DroneState, sp: Setpoint, dt: float) -> RcCommand:
        ...
        self._it = now
        if self.wind is not None:
            # снимок устойчивого hold для общего трима (wind_trim.py п.2): гвоздь,
            # обе станции не тормозят, стики в центре, обе оси учатся, |v| VINS
            # < steady_v И канал IPM (если годен) не видит движения
            steady = (not stick and self._pinx is not None
                      and not self._st_fwd.braking and not self._st_rgt.braking
                      and self._learn_fwd and self._learn_rgt
                      and math.hypot(s.vins_vx, s.vins_vy) < self.wind.steady_v
                      and not (s.ipm_ok and math.hypot(s.ipm_vfwd, s.ipm_vlat)
                               > self.wind.steady_v))
