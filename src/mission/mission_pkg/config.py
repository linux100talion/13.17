...
class BootstrapConfig:
    ...
    kf_alt_max: float = None         # None = дефолт оценщика (0.06 = 6% высоты ≈ 0.19 м
                                     # на трёх метрах). Ушла высота больше — опора
                                     # пересевается, а частичный сегмент ВЫБРАСЫВАЕТСЯ как
                                     # недостоверный (масштаб сцены зависит от высоты), и
                                     # накопитель положения не наполняется. Поднятие порога —
                                     # опыт на причинность связи «высота → горизонтальный
                                     # уход», а не рабочая настройка: с большим порогом в
                                     # накопитель поедет мусор по масштабу.
