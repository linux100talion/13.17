...
class BootstrapConfig:
    ...
    roll_ki: float = 3.0             # СВЕДЕНО с тем, что реально летало: в прогонах
                                     # (K1s…R7s) жило BS_ROLL_KI=3 / IMAX=150 / SMOOTH=25,
                                     # а в конфиге стояли 2 / 120 / 1. Все замеры оси —
                                     # на лётных значениях, поэтому дефолтом становятся
                                     # они; иначе кто-то запустит без env и получит
                                     # НЕизмеренный контур.
    roll_kd: float = 0.0
