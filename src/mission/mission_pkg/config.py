...
class BootstrapConfig:
    ...
    ipm_max_speed: float = 8.0       # потолок ПРАВДОПОДОБИЯ кадра, М/С. Выше — кадр не
                                     # командует (не подрезается). Взято ВЫШЕ худшего
                                     # реального разгона (6.7 м/с у сорвавшегося YW2s3):
                                     # гейт обязан ловить мусор, а не глушить ось ровно
                                     # тогда, когда борт действительно понесло. 0 = выкл.
    ipm_alt_band_fwd: float = 0.5    # ПРОДОЛЬНАЯ ось: полоса «высота установилась», М.
                                     ...
                                     # 0.5 м = ВЫШЕ болтанки удержания (0.2–0.4 м на трёх
                                     # метрах) и НИЖЕ размаха набора (3 м). ⚠️ Раньше здесь
                                     # стоял порог по вертикальной СКОРОСТИ (0.35 м/с) —
                                     # серия IG1s: дребезг у порога глушил тангаж весь полёт,
                                     # борт унесло на 39 м. Судим по высоте, см. _AltSettled.
                                     # 0 = выкл.
    ...
    ipm_arm_frames: int = 5          # сколько кадров подряд должно пройти все гейты, чтобы
