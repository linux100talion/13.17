...
class BootstrapConfig:
    ...
    #   ipm_ok И |vins_v−ipm_v| > ipm_tol (стойко sane_n кадров) → расхождение с
    #     НЕЗАВИСИМЫМ оптическим каналом IPM (демпфер яруса 0) = ошибка масштаба.
    # Гейт двусторонний: не sane → ярус 1 падает на 0 (демпфер, VINS не нужен) →
    # унос невозможен. 0 = выкл (старое поведение). Восстановление — /restart.
    vins_v_max: float = 12.0      # м/с, физ. потолок |vins_v| (0 = выкл)
    vins_ipm_tol: float = 4.0     # м/с, допуск |vins_v − ipm_v| при живом IPM (0 = выкл)
    vins_sane_n: int = 3          # кадров подряд расхождения с IPM до демоута
