...
class BootstrapConfig:
    ...
    ipm_derot: float = +1.0          # вычитать вращательное поле по гироскопу из канала
                                     # вида сверху: 0 = нет, ±1 = знак. Медиана смещений
                                     # считает поле «только сдвиг», поэтому разворот на dψ
                                     # читается как боковой снос X·dψ (полоса лежит в 3-6 м
                                     # ВПЕРЕДИ — плечо больше самого разворота). ω_up берём
                                     # у гироскопа, а не оцениваем по полосе: плечо 3→6 м
                                     # для оценки разворота слабое. Замер по I1s1 (стенд
                                     # att_extrap_test.py, боковая ось, модель exact):
                                     #   derot   утечка ωx, м    R²    СКО ошибки
                                     #      0        +1.99      0.68      0.24
                                     #     +1        -0.29      0.90      0.13
                                     #     -1        +4.25      0.46      0.48
                                     # Продольный канал не тронут (масштаб 0.91, ошибка
                                     # 0.40 во всех трёх). Знак выбран замером, а не
                                     # выводом — в этой кампании два знака подряд были
                                     # выведены неверно.
