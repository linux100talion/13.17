...
class BootstrapConfig:
    ...
    joy_signs: str
    pilot_stale: float        # СТОРОЖ СВЕЖЕСТИ ПУЛЬТА, с: источник стиков молчит
                                     # дольше — нода ОТПУСКАЕТ override (ch1..4 → 0,
                                     # RC_RELEASE) вместо публикации команды, и FCU
                                     # возвращается к физическому приёмнику. Без
                                     # сторожа пропажа пульта = полёт с замороженными
                                     # стиками: адаптер держит последние значения, а
                                     # RC-failsafe полётника не сработает — под
                                     # активным override radio_in жив нашей же рукой
                                     # (laptop_move.md §5.2). 0 = ВЫКЛ (поведение до
                                     # 2026-09-22). ⚠️ Больше периода источника с
                                     # запасом: /joy идёт 20 Гц (autorepeat_rate),
                                     # 0.5 с = 10 кадров; у RosPilot период задаёт
                                     # стрим-рейт FCU. domain/pilot_link.py.
                                     # BS_PILOT_STALE
    # фазы
    alt: float
    throttle_climb: int
    ...
    rth_dyaw_tol: float       # УСТОЙЧИВОСТЬ Δyaw как признак зрелости VINS, градусы
                                     # (0 = чек выкл). Разность курсов «AHRS − VINS»
                                     # (brdn= в статусе) у сошедшегося кадра СТОИТ: борт
                                     # крутится — растут оба курса одинаково. Пока гуляет
                                     # больше tol, кадр не устоялся: ни дом латчить, ни
                                     # тем более сажать на него курс полётника.
                                     # BS_RTH_DYAW_TOL
    rth_dyaw_wz: float        # ПОТОЛОК |скорости разворота| для чека выше, °/с
                                     # (0 = не гейтить). На вираже курс AHRS и курс
                                     # VINS приходят разными трактами с разной
                                     # задержкой, и Δyaw гуляет ЛАГОМ, а не уходом
                                     # (171337: размах 5.1° при 41.6 °/с против 2.4°
                                     # при 9.5 °/с в том же полёте) — крутимся быстрее,
                                     # чек молчит. Родня — ipm_wz_gate деротации.
                                     # BS_RTH_DYAW_WZ
    ekf_yaw_src: str          # ЧЕЙ КУРС ДЕРЖИТ EKF: 'compass' — как всегда
                                     # (EK3_SRC1_YAW=1); 'vins' — после латча возврата
                                     # переключаем на 6 (ExternalNav): курс ведёт наша
                                     # поза. Приращения у обоих от ОДНОГО гироскопа —
                                     # меняется источник КОРРЕКЦИИ дрейфа: компас
                                     # (абсолютен, но шумит и врёт над железом) →
                                     # визуальные фичи (гладко, без помех, но без
                                     # абсолюта). Механика и риски — src/nav/frames.md.
                                     # BS_EKF_YAW_SRC
    ekf_yaw_fallback: float   # 1 = при разрыве (перерождение / не sane / закрытый
                                     # мост / протухший поток) вернуть EK3_SRC1_YAW=1.
                                     # Без этого EKF остался бы на голом гироскопе.
                                     # BS_EKF_YAW_FALLBACK
    rth_home_settle: float    # СКОЛЬКО СЕКУНД ЖДАТЬ спокойной рамы перед тем, как
                                     # поставить дом: на первом открытии моста ray_tracer
                                     # нарочно отдаёт сырой VINS (anchor_open_reset_sec),
                                     # и полётник СКАЧКОМ пересаживается на нашу раму —
                                     # дом, поставленный до скачка, тут же устареет
                                     # (103244: «до дома 10 м» на неподвижном борте).
                                     # Обязано быть ≥ anchor_open_reset_sec. Скачок в
                                     # выдержке перезапускает отсчёт. BS_RTH_HOME_SETTLE
