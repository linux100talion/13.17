...
class BootstrapConfig:
    ...
    ipm_derot: float = 0.0           # ⚠️ ВЫКЛ ПО ЗАМЕРУ. Вычитать вращательное поле по
                                     # гироскопу: 0 = нет, ±1 = знак.
                                     # ЗАМЫСЕЛ. Медиана смещений считает поле «только
                                     # сдвиг», поэтому разворот на dψ читается как боковой
                                     # снос X·dψ (полоса в 3-6 м впереди — плечо больше
                                     # самого разворота). Офлайн вычитание работало
                                     # отлично: утечка ωx +1.99 → −0.29, R² 0.68 → 0.90,
                                     # СКО ошибки 0.24 → 0.13 (стенд по I1s1).
                                     # ПОЧЕМУ ВСЁ РАВНО ВЫКЛ. У ω_z есть СМЕЩЕНИЕ НУЛЯ:
                                     # гироскоп против истины по одометрии, рад/с —
                                     #   I1s1 −0.0236   J2s1 −0.0060   J2s3 −0.0141
                                     #   L2s1 −0.0282   L2s2 −0.0242   L2s5 −0.0122
                                     # Зрение гироскоп раньше не использовало, и это
                                     # никого не трогало. Вычитание умножает смещение на
                                     # плечо полосы 4.5 м и вливает в измерение боковой
                                     # скорости −0.03…−0.13 м/с. Демпфер СКОРОСТИ зануляет
                                     # то, что видит, значит гонит борт ровно с этой
                                     # скоростью. Серия L2s: уход 6.64 ± 4.72 м, вбок
                                     # −4.78 ± 4.48 (ВСЕ ПЯТЬ в одну сторону) против
                                     # J2s 1.75 ± 0.54 и +0.37 ± 0.98 без вычитания.
                                     # Разброс серии — оттого, что смещение своё у каждого
                                     # прогона.
                                     # ⚠️ УРОК: R² и СКО ошибки СДВИГ НЕ ВИДЯТ. Для
                                     # демпфера скорости сдвиг опаснее разброса — стенд
                                     # теперь печатает его отдельной колонкой.
                                     # Включать только вместе со снятием смещения ω_z
                                     # (например ФВЧ по ω_z) и новым замером сдвига.
