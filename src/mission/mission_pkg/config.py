...
class BootstrapConfig:
    ...
    # кап И-члена (трима), PWM. Держим под УРОВНЕМ ВЕТРА (~44 PWM в этом мире):
    # на первом торможении трим учит ветер, но при капе 100 мотал бы полный
    # импульс → перелёт и возврат назад −4.6 м (ab_dpv_trim). Кап 50 ≈ ветер →
    # первое обучение насыщается у ветра, возврат мал; удержания хватает.
    dpvins_imax: float = 50.0
