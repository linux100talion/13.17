...
class BootstrapConfig:
    ...
    ipm_vz_max_fwd: float = 0.35     # ПРОДОЛЬНАЯ ось: не доверять каналу, пока |vz| выше,
                                     # М/С. Полоса земли лежит ВПЕРЕДИ (X≈3…6 м), поэтому
                                     # ход по высоте читается как ход вперёд: замер по 8
                                     # прогонам — 4.5–7.2 м/с фантома на взлёте при
                                     # истинных ≤1.3, тангаж в насыщении 22–43% первых
                                     # трёх секунд. 0 = выкл.
    ipm_vz_max_lat: float = 0.0      # БОКОВАЯ ось: по умолчанию ВЫКЛ. Геометрия её почти
