...
def ready_verdict(ready, age, seen, uptime, fresh_sec, wait_sec):
    """Что подать в `BridgeGate.on_odom(ready=…)` по вердикту зрелости лётной ноды.

    Разделяет ДВА разных «вердикта нет»:
      - нода ещё не заговорила (мы только запустились) → False, мост ЗАКРЫТ. Иначе
        выходит стартовая дыра: разбор 192430 — 35 поз ушли в EKF за 3.4 с до первого
        /vins/bridge_ok, причём рама была залатчена от позы КРУТЯЩЕГОСЯ НА ЗЕМЛЕ борта
        (в мире нет трения, борт держит только ветер: на спавне diagonal он доворачивал
        на 7–9° до отрыва). Курс EKF утащило, а на настоящем открытии latch_yaw взял
        угол ИЗ КУРСА EKF и наследовал ошибку: +89.1° вместо +96…+100°, и +10…+12°
        ошибки курса держались весь полёт;
      - ноды нет вовсе (голый Orin со стримером) → None, мост живёт своими проверками.
        Отличаем по времени: не заговорила за wait_sec — значит её и нет.

    ready/age — последний вердикт и его возраст; seen — приходил ли вердикт хоть раз;
    uptime — сколько живём; fresh_sec — срок годности вердикта; wait_sec — сколько
    ждём первого (0 = не ждать, поведение до 2026-09-09).
    """
    if seen and age < fresh_sec:
        return bool(ready)
    if not seen and uptime < wait_sec:
        return False
    return None
