...
class FrameAnchor:
    ...
    def update(self, vins_pos, vins_yaw, ekf_pos, ekf_yaw, now):
        ...
        if self.relatch_m > 0 and dn > self.relatch_m:
            # ЖЁСТКАЯ ПОДТЯЖКА ПРАВИТ ТОЛЬКО ТРАНСЛЯЦИЮ (2026-09-09, разбор 173415).
            # Расход, который её будит, — величина В МЕТРАХ; выводить из него УГОЛ
            # мы не обязаны, а цена ошибки несимметрична: кривая трансляция даёт
            # постоянный сдвиг, кривой угол ПОВОРАЧИВАЕТ ВСЮ КАРТУ вокруг якоря
            # (на 45 м ошибка 20° = 15 м) и при EK3_SRC1_YAW=6 уходит прямо в курс
            # полётника. В 173415 VINS подвис на 1.96 с (штампы непрерывны, данные
            # целы — обычный подвис счёта), подтяжка сработала на первом же
            # сообщении после подвиса и защёлкнула ω·лаг = 10 °/с · 2 с = 20°
            # вместо верного угла — на весь полёт. Угол задаётся ТОЛЬКО при
            # рождении рамы: первый латч, latch_yaw на открытии моста, перерождение.
            self.t = ekf_pos - self.rotate(vins_pos)
            self.relatch_n += 1
            self._latch_wall = now
            return 'relatch'
