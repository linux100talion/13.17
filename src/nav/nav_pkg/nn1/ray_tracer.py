            ...
            if gate_open != self._gate_open:
                ...
                if gate_open:
                    ...
                    self.get_logger().warn(
                        ...
                        f"якорь заморожен")
            # шестое поле — Δyaw ЯКОРЯ в градусах: на сколько повёрнут кадр VINS
            # относительно кадра EKF (а он выровнен компасом). Это ровно «разность
            # VINS и компаса»: латчится при созревании, дальше замирает и меняется
            # только на жёстких подтяжках/перелатчах — по её скачкам видно
            # перерождение рамы, по медленному ходу — уход компаса или курса VINS.
