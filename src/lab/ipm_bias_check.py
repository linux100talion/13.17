...
print('%-7s| %-10s | %8s | %8s | %8s | %8s | %6s'
      % ('прогон', 'ось', 'сдвиг', 'изм.ср', 'ист.ср', 'СКО ош', 'corr'))
for b in sys.argv[1:]:
    u = '/out/%s_bag' % b
    ...
    for name, src, meas in (('продольная', np.array(d8) if d8 else None,
        ...
        print('%-7s| %-10s | %+8.3f | %+8.3f | %+8.3f | %8.3f | %+6.2f'
              % (b, name, np.mean(v[ok] - t_[ok]), v[ok].mean(), t_[ok].mean(),
                 np.std(v[ok] - t_[ok]), c))
