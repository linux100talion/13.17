...
| `{"land": 0\|1}` | кнопка посадки SA (`/joy.buttons[--land-btn]`, дефолт 0 = `BS_LAND_JOY=b0` ноды): 1 = нажата → при `rel_alt ≤ 2 м` и `|v| ≤ 1 м/с` freefly уходит в `SoftLand` (мягкая посадка). Уровень: держится до `{"land": 0}` |
