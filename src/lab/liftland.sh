...
# YAW-hold (фаза 2): визуальный курс-холд
[ "${BS_YAWHOLD:-0}" = "1" ] && ARGS+=(--yaw-hold)
[ -n "${BS_YAWH_KP:-}" ]    && ARGS+=(--yaw-kp "$BS_YAWH_KP")
[ -n "${BS_YAWH_KI:-}" ]    && ARGS+=(--yaw-ki "$BS_YAWH_KI")
[ -n "${BS_YAWH_IMAX:-}" ]  && ARGS+=(--yaw-imax "$BS_YAWH_IMAX")
[ -n "${BS_YAWH_MAX:-}" ]   && ARGS+=(--yaw-max "$BS_YAWH_MAX")
...
[ -n "${BS_RE_GAP:-}" ]    && ARGS+=(--roll-excite-gap "$BS_RE_GAP")
[ -n "${BS_RE_SETTLE:-}" ] && ARGS+=(--roll-excite-settle "$BS_RE_SETTLE")
