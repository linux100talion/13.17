...
# съезжает (i — это и есть номер в имени), поэтому досъёмка неотличима от непрерывной
# серии. ⚠️ Только если конфиг тот же: досъёмка ЧУЖИМ конфигом молча смешает две точки.
for i in $(seq "${SEQ_START:-1}" "$N"); do
    NAME="${PREFIX}${i}"
