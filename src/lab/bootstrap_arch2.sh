...
# тормоз с момента отпускания (−brake·v без точки) и гвоздь по таймауту (config.dpvins_settle_brake / dpvins_pin_t)
[ -n "${BS_DPVINS_SETTLE_BRAKE:-}" ]   && ARGS+=(--dpvins-settle-brake "$BS_DPVINS_SETTLE_BRAKE")
[ -n "${BS_DPVINS_PIN_T:-}" ]          && ARGS+=(--dpvins-pin-t "$BS_DPVINS_PIN_T")
