...
ХВОСТ БРЕЙКА КАК У ДЕМПФЕРА (2026-09-06, --brake-t -1: трим в брейке НЕ заморожен, стоит
  ...
  (ki 8, brake_t −1), полёт cmd/4 — ДОКАЗАН висением (2.36 против 4.6–5.5).
TWIST (cmd/6, --tau-meas 0.11 = скорость из одометрии вместо разности+EMA; лаг измерен
  vins_twist_check.py: 0.00 против 0.14 по штампам, + приход ~0.11): ki 8 brake 5 кап 2 bt −1:
  kp 32 → 2.33/0.04/vmax 1.40; kp 40 → 2.15/0.02/1.25; kp 60 → 2.49/0.01/0.86; kp 90 →
  2.27/0.01/0.76 — kp режет СКОРОСТЬ (разгон через гвоздь / возврат на тримe-излишке, bag
  130326: 1.2 м/с против 0.7 у демпфера), пик почти нет; окно ki шире ~20 % (ki 12 → 2.56,
  15 → 3.81 против 3.83/5.57 при τ 0.35). Стенд не знает шума twist и σθ — полёт.
