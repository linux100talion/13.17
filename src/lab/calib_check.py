...
def report_bias(od, imu, cmd_r, cmd_p, segs):
    """Уход объекта при стиках в центре — ПО ОБЕИМ ОСЯМ и по полному пути.

    ⚠️ Раньше здесь считалась ТОЛЬКО продольная ось, и это дало ложный вывод: прогон
    `A4d_gbias_min` показал падение продольной скорости 14.3 → 0.63 м/с, был объявлен
    успехом, а полный путь при этом остался прежним (160 → 153 м) — дрон уехал ровно
    столько же, но вбок. Рампа крена там вообще не изменилась (0.176 → 0.184 °/с).
    Судить об уходе по одной оси нельзя: помеха свободно перетекает между ними.

    Окно — ТОЛЬКО ДО первого командного сегмента. После сегмента борт летит с набранной
    скоростью, а посадка добавляет свой наклон: усреднять по всему «стики в центре» —
    значит намерить не смещение, а последствия команды.
    """
    if len(cmd_p) == 0:
        return
    ...
    if still.sum() < 20:
        print("\nуход объекта: нет участка со стиками в центре выше 1 м до первой команды")
        return
    s = od[still]
    t0, t1 = s[0, 0], s[-1, 0]
    dur = max(1e-6, t1 - t0)
    bv = body_vel(od, t0, t1)
    print(f"\nУХОД ОБЪЕКТА (стики в центре, до первой команды t<{t_cut:.1f}, "
          f"{still.sum()} проб, {dur:.1f} sim-сек):")
    print(f"  {'ось':<8}{'рампа оценки':>15}{'угол истина':>14}{'ускорение':>13}"
          f"{'v в конце':>12}")
    # рампа = скорость расхождения «истина − оценка FCU» по каждой оси; ускорение и
    # скорость — в теле (vel_idx 1 = вперёд, 2 = вбок), из позиции.
    for lbl, col, icol, vi in (("крен", 4, 1, 2), ("тангаж", 5, 2, 1)):
        ramp = float('nan')
        if len(imu):
            est = np.interp(s[:, 0], imu[:, 0], imu[:, icol])
            ramp = math.degrees(np.polyfit(s[:, 0] - t0, s[:, col] - est, 1)[0])
        a = np.polyfit(bv[0], bv[vi], 1)[0] if bv else float('nan')
        vend = bv[vi][-1] if bv else float('nan')
        print(f"  {lbl:<8}{ramp:>12.3f} °/с{math.degrees(np.mean(s[:, col])):>13.2f}°"
              f"{a:>10.3f} м/с²{vend:>9.2f} м/с")
    dx, dy = s[-1, 1] - s[0, 1], s[-1, 2] - s[0, 2]
    vh = math.hypot(*(np.gradient(s[:, 1], s[:, 0])[-3:].mean(),
                      np.gradient(s[:, 2], s[:, 0])[-3:].mean()))
    print(f"  ИТОГО горизонтальный путь {math.hypot(dx, dy):.1f} м, "
          f"|v| в конце {vh:.2f} м/с   ← главная метрика, а не одна ось")
    print(f"  курс уехал на {math.degrees(s[-1, 6] - s[0, 6]):+.0f}° "
          f"({math.degrees(s[-1, 6] - s[0, 6]) / dur:+.2f} °/с) — третья ось, тоже считается")
    if len(imu):
        mi = (imu[:, 0] >= t0) & (imu[:, 0] <= t1)
        if mi.sum() > 20:
            print(f"  FCU считает: крен {math.degrees(np.mean(imu[mi, 1])):+.2f}°  "
                  f"тангаж {math.degrees(np.mean(imu[mi, 2])):+.2f}°   "
                  f"← расхождение с истиной = причина ухода (ToDo факт 2d)")
