...
def simulate(a):
    ...
    # ТЕРМИНАЛЬНАЯ vs ТРАНЗИЕНТНАЯ: растёт ли |v| в конце ОТНОСИТЕЛЬНО начального
    # выброса. Устойчивый (даже шумный) демпфер: v_term << транзиент → ratio<1.
    # Расход (osign-1/раскачка): |v| в конце >> транзиента → ratio>>1. Робастно к
    # осцилляции через ноль и к шумовому джиттеру при малом RMS_v.
    t3 = np.array_split(idx, 3)
    a1 = float(np.max(np.abs(vs[t3[0]])))
    growth = v_term / max(1e-6, a1)
    if growth > 1.3:
