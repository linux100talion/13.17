...
#   LV=2           — «GPS ОТСУТСТВУЕТ С БУТА» (модель реального борта без
...
# all/fresh-start) — свежий клон летит штатным профилем сразу. Существующий
# .env не трогаем. Строки KEY=VALUE применяются ТОЛЬКО
# к незаданным переменным — env снаружи (`LV=1 bash ...`) всегда сильнее.
# Так `bash src/lab/freefly_lv.sh` без ничего летит штатным профилем бокса
...
export TOPICS_EXTRA="${TOPICS_EXTRA:-/joy /mavros/state /mission/status /feature /odometry /model/iris_cam/odometry /flow_dbg /flow_dbg2 /flow_dbg6 /flow_dbg7 /flow_dbg8 /flow_dbg9 /flow_dbg10 /mavros/imu/data /nn1/bridge /vins/sane /vins/bridge_ok /mavros/vision_pose/pose /mavros/vision_speed/speed_twist}"
...
# Варп в bag не пишется — ipm_video.py пересчитывает его ЛЁТНЫМ FlowEstimator и
