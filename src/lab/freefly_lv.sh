...
# ── ГЕЙНЫ ДЕМПФЕРА: kp 30 → 90 (шаг 1), ki 30 → 60 (шаг 1b) ─────────────────
...
# ШАГ 1b: ki 30 → 60 — долг вдвое дешевле. Затухание платит только √2:
# ζ ≈ (kp/2)·√(α/ki) = 0.70 против 0.33 на исходных 30/30, то есть даже после
# удвоения ki запас БОЛЬШЕ, чем был до всей серии; ω_n 0.66 → 0.94 рад/с.
# Контрфакт по модели обоих бэгов: путь за 5 с 1.2-1.3 → 0.2-0.6 м.
# ⚠️ Выше не лезть без нового замера: τ_s (запаздывание канала) на чистом висении
# у земли 0.60 с, а на прогоне с набором до 4.5 м и манёврами — 1.05 с, и по
# пессимистичной идентификации ki=90 уже звенит (пик 1.09), ki=120 разносит.
# ki=60 устойчив по ОБЕИМ идентификациям — потому и он.
