...
Под `freefly_lv.sh` в архив прогона едет **копия**, а сам каталог остаётся в
`docker/sim/output/scene_bag` до следующего прогона: `ros2 bag play` и всё с
дефолтным `SCENE_BAG` работают сразу после записи. Цена — двойной объём (2+ ГБ)
на время между прогонами; следующий прогон чистит `output/scene_bag*`, архив цел.
Чистится при этом **только содержимое** — сама папка `scene_bag` не удаляется
НИКОГДА (пустая между прогонами — норма): её inode должен пережить прогон, иначе
терминал с `cd` внутрь и любой держащий каталог процесс окажутся в удалённой
директории. Rosbag2 (Humble) в существующий каталог писать отказывается («Output
folder 'scene_bag' already exists», флага overwrite нет), поэтому запись идёт во
временный `output/.rec/scene_bag` (имя то же → штатные `scene_bag_0.db3` +
`metadata.yaml`), а на стопе записи файлы **переезжают** в постоянную папку —
rename на том же ФС, мгновенно. Проверено: inode папки до и после совпадает,
`ros2 bag info/play` читают результат.
...
Взлёт в **ALT_HOLD** и инициализация VINS в полёте (без GPS), под целевую
...
| `vins_sane_replay.py <bag>` | **ГЕЙТ ЗДОРОВЬЯ VINS** по bag: настоящий `Handover.vins_sane` (потолок / физика висения / занижение против IPM) на сетке 0.05 с — скорость VINS по штампам (`VinsTrack`), IPM из `/flow_dbg8\|9`, высота перцепции и стики из `/mission/status`, рядом истина Gazebo. Печатает фронты sane→insane с причиной и срабатывания чека занижения; ручки гейта — аргументами. Так валидирован чек занижения (114248 — срабатывание на 106 с; контрольные и серии 09-03/04 — 0). С 2026-09-09 кормит гейт и РЕЖИМОМ FCU из `/mavros/state` (в RTL/SMART_RTL/AUTO/GUIDED чеки «по центру стика» молчат — `modes.navigates`); `--no-fcu-mode` = как гейт судил ДО фикса, для A/B. На bag возврата 044105: до — 6 фронтов / 24.8 с insane (5 из 6 `hover` при |vins_v| ≈ |ipm_v| ≈ истина 3–4 м/с), после — 0 | `python3 /lab/vins_sane_replay.py /root/sim_ws/output/joystick/<RUN>/bag [--scale-ratio 0.5 …]` |
...
> ⚠️ **Twist истины Gazebo (`/model/iris_cam/odometry`) — В ОСЯХ ТЕЛА** (child_frame): x вперёд, y влево. Проверка 2026-09-06 по Δpos/dt: ошибка 0.02 м/с в теле против 3.1 в мире. Модуль |v| от рамы не зависит (gust_hold_compare, brake_phase — верно), а всё, что крутит компоненты по курсу, обязано знать раму: `stick_lateral.py` берёт vx/vy как есть, `vins_twist_check.py` поворачивает истину в мир по курсу. До поправки stick_lateral крутил тело ещё раз (на курсах у 180° знак «вперёд» переворачивался) — числа сноса 113224 в cmd/5 с этой ошибкой.
...
| `release_stop.py` | **СТОП ПОСЛЕ ОТПУСКАНИЯ СТИКА** по фазе станции (`brk=`): на каждое отпускание — ярус, |v| при отпускании, время и путь до гвоздя, |v| при гвозде, уход за 5 с после, вираж (dyaw за 3 с); «гвоздь не взят» = пилот взял стик раньше. 160730: DpHold 1.5–2.4 с / 3–5 м, DpVins 5–12 с / 7–16 м → cmd/8 settle_brake + pin_t | `docker exec p1317_nav bash -lc "source …; python3 /lab/release_stop.py /root/sim_ws/output/joystick/<RUN>"` |
| `ipm_alt_quality.py` | **КАЧЕСТВО КАНАЛА ВИДА СВЕРХУ ПО ВЫСОТАМ** — годится ли IPM опорником для чека занижения VINS выше 4 м: окна висения на демпфере по плато высоты, доля годных кадров, гейн к истине (МНК, тело), корреляция, шум, VINS/истина. 171454: 100 % годных до 23 м, гейн 0.5–0.77, шум 0.16 (10 м) → 0.41 м/с (23 м) → alt_max 4 → 25 при ipm_min 1.2 (реплей: 0 ложных на 6 bag, эмуляция ×0.2 `vins_sane_replay.py --vins-scale` ловится на плечах) | `docker exec p1317_nav bash -lc "source …; python3 /lab/ipm_alt_quality.py /root/sim_ws/output/joystick/<RUN>"` |
...
| `ipm_band_ab.py` | стенд **ГЕОМЕТРИИ ПОЛОСЫ IPM** — A/B ширины (`yhalf`), длины/дальности (`x0..x1`) и лимита фич на ОДНИХ кадрах bag, лётным `FlowEstimator` в лётном конфиге (`BS_*` + мета `.env` прогона): коды брака, углы полосы за кадром (чёрные клинья), скорости против истины Gazebo и СЫРОЙ сигнал (приращение пути за кадр − истина·dt) с разложением на Δpitch/Δroll/ω_z/масштаб + попарная разница с базой (= пиксельная часть). Ответ 2026-08-28 (yaw/swing/1): ширина — НЕ рычаг (разница 0.2–0.6 мм при ошибке 7 мм/кадр), дальше/длиннее — хуже ∝1/sin²α; шум = тайминг углов. Читает bag из sqlite без `rosbag2_py`/`mavros_msgs` → бегает и на ХОСТЕ (venv с cv2 + `source /opt/ros/jazzy/setup.bash`) при погашенном nav. Env: `IB_BAG`, `IB_VARIANTS` («yhalf:feats:x0:x1,…»), `IB_ALT_SRC` (latch\|true), `IB_DETAIL`, `IB_CSV` | `IB_BAG=docker/sim/output/joystick/<RUN>/bag python3 src/lab/ipm_band_ab.py` |
| `bag_rviz.sh <прогон\|bag>` | **ПРОСМОТР BAG'а В RVIZ2 НА ХОСТЕ** (jazzy, стек сима может жить): видео `/image_color`, стрелки носа (Odometry, x тела) и линии пути истины Gazebo (зел.), VINS (красн., в СВОЁМ `world` — точка init + курс первого кадра) и EKF (голуб., frame `map` → статический TF `world→map`), Fixed Frame `world`, сохранённый вид Top-down. `/path` в bag нет — линии строит `bag_path_pub.py` (Odometry/PoseStamped → `nav_msgs/Path`, 5 Гц, прореживание 5 см); конфиг `bag_view.rviz`. Всё в СВОЁМ ROS-домене (`DOMAIN` 42): живой Gazebo в домене 0 светит на хост `/clock` и `/model/iris_cam/odometry`, с `ros2 bag play --clock` у RViz два `/clock` вперемешку → «Detected jump back in time» каждый кадр + живой борт поверх реплея. `--clock`/`use_sim_time` не нужны: штамп записи = wall-эпоха, заголовки = sim-время, TF только статический. Env: `RATE`, `START` (с), `LOOP`, `RVIZ_CFG`, `DISTRO`; хвост аргументов → `ros2 bag play` | `RATE=2 bash src/lab/bag_rviz.sh docker/sim/output/joystick/<RUN>` |
| `anchor_replay.py <прогон\|bag>` | **A/B СПАРИВАНИЯ ПОЗ ЯКОРЯ** — «по приходу» против «по штампу»: гоняет ДВА `FrameAnchor` по одному bag и печатает РАЗЪЕЗД ПАРЫ (|последняя поза EKF − поза на момент штампа кадра|, цена лага в метрах), РАСХОД |EKF − map(VINS)| на каждом обновлении и число жёстких подтяжек. ⚠️ Мерить лаг по bag можно только СНЯВ расхождение ТЕМПА sim/wall (1 % за полёт = секунда мнимой задержки). Вердикт 2026-09-09 по 181233/181936/173415: спаривание по штампу ХУЖЕ (расход 0.083 → 0.107 и 0.114 → 0.264 м, подтяжек 1 → 3) — поза EKF не независимый наблюдатель, а отклик на нашу же `vision_pose`, которая уходит со штампом «сейчас»; отсюда `anchor_pair_by_stamp=False` | `docker exec p1317_nav bash -lc "source …; python3 /lab/anchor_replay.py /root/sim_ws/output/joystick/<RUN>"` |
| `yaw_ab.py <прогон> [<прогон> …]` | **СУДЬЯ A/B «ЧЕЙ КУРС ДЕРЖИТ EKF»** (кампания `cmd/yaw_ab`): одна таблица по нескольким прогонам — какой источник курса реально стоял (`yawsrc=`), Δyaw якоря после РАБОЧЕГО открытия моста и его РАЗМАХ за полёт (обязан быть 0.0°: угол задаётся при рождении рамы, подтяжка его не трогает — ненулевой размах = перерождение VINS или регресс фикса 173415, такой прогон в A/B не годится), ОШИБКА КУРСА |EKF − истина Gazebo| медиана/90 % **только на спокойных участках** (`--wz`, по умолчанию 5 °/с — на вираже сравнение меряет разницу задержек трактов, а не источник), дрейф EKF и промах по истине. Числа сравнимы только при схожей схеме полёта — протокол в `cmd/yaw_ab/README.txt` | `python3 src/lab/yaw_ab.py docker/sim/output/joystick/<RUN1> <RUN2> …` |
| `rth_check.py <прогон\|bag>` | **ПРОМАХ ВОЗВРАТА ДОМОЙ** (прогон `cmd/rth`, RTL полётника к home на EKF-от-VINS): точка отрыва и точка касания глазами истины Gazebo и глазами `/mavros/local_position/pose` → три числа — ПРОМАХ ПО ИСТИНЕ (что увидел бы человек на поле), ПРОМАХ ГЛАЗАМИ EKF (у RTL обязан быть ~0: летит к своей цифре `home`) и ДРЕЙФ EKF на момент касания (цена возврата = дрейф + перелатчи рамы FrameAnchor). Кадры `map` и `world` оба ENU и на север — вычитаются без поворота, привязка к моменту отрыва. С хоста (jazzy) — геометрия; в контейнере nav ещё и лента режимов FCU из `/mavros/state` (когда включился RTL, сколько вёл) Плюс СЛЕД: расстояние от каждой точки обратного плеча до ближайшей точки плеча «туда» (медиана/90%/максимум) — мера «размотал свой путь» для `cmd/smart_rth` (SMART_RTL) и «совпала ли прямая домой с уходом» для `cmd/rth`; границу плеч берёт по фронту RTL/SMART_RTL, а без разбора режимов — по самой дальней точке | `python3 src/lab/rth_check.py docker/sim/output/joystick/<RUN>` |
