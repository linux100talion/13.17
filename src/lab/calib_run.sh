...
# ПРОВОДКА РУЧЕК — до подъёма стека. Ручка, заданная в BS_*, но не доезжающая до ноды,
# роняет прогон ЗДЕСЬ за две секунды. Иначе она молча улетает на дефолте: значение попадает
# в мету (выше), прогон выглядит настроенным, и полтора часа стенда дают правдоподобный
# ложный вывод — ровно так пропал свип B3s (BS_ROLL_RATE_KI=5 → фактически 0).
bash "$SCRIPT_DIR/check_knobs.sh" || exit 4
