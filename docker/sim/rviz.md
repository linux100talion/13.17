...
трёх независимых источников (плюс служебная четвёртая — что из них едет в полётник). Плюс отдельное окошко с видео камеры.
...
| 🟠 **четвёртая, `/vins/path_fcu`** | **ЧТО РЕАЛЬНО ЕДЕТ В EKF** — поза VINS, уже повёрнутая якорем в кадр полётника (`Rz(Δyaw)·p_vins + t`). Голубая тянется именно к НЕЙ, а не к красной | `/mavros/vision_pose/pose` |
...
### Почему голубая (EKF) подтянулась НЕ к красной, а «в другую сторону»?

Так и должно быть — если спавн не на восток. Полётник **никогда не видит красную**: в
EKF едет поза VINS, ПРОПУЩЕННАЯ ЧЕРЕЗ ЯКОРЬ, `vision_pose = Rz(Δyaw)·p_vins + t`. А
красная в RViz нарисована в СЫРОМ кадре VINS, без этого поворота. Значит голубая
обязана лечь на красную, **повёрнутую на Δyaw вокруг якоря**, — а это совсем другое
место на экране.

Замер по паре полётов 2026-09-09 (одна и та же сборка, разница только в точке спавна):

| прогон | Δyaw якоря | \|EKF − СЫРОЙ VINS\| | \|EKF − VINS, повёрнутый якорем\| |
|---|---|---|---|
| 181233, спавн на восток | −4.6° | медиана 2.35 м | медиана **0.15 м** |
| 181936, `SPAWN_POSE=diagonal` | +96.7° | медиана **44 м** (до 112 м) | медиана **0.19 м** |

То есть подтяжка в обоих случаях идеальная (15–19 см), просто на востоке Δyaw ≈ 0 и
поворот незаметен — голубая ложится прямо на красную и всё выглядит «правильно». На
диагонали тот же самый механизм разводит линии на 44 м, и глазами это читается как
«EKF уехал не туда».

Чтобы не гадать, `bag_path_pub.py` рисует **четвёртую линию `/vins/path_fcu`** —
ровно то, что уходит в полётник. Голубая обязана лежать на ней. Если НЕ лежит —
вот это уже настоящая беда: значит EKF не берёт нашу позу (закрыт мост, фьюжн не
поднялся, поза за инновационным гейтом).
...
- Линии (`/truth/path`, `/vins/path`, `/ekf/path`, `/vins/path_fcu`) рисует `bag_path_pub.py` — RViz из
