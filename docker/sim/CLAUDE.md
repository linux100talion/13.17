...
rx.md                       — КАРТА ПУЛЬТА (TX12): какой канал/орган за что отвечает —
                              стики CH1..4, CH5 = FLTMODE (не читаем), CH6/SC = потолок
                              лесенки, CH7/SF = мастер сырых стиков, CH8 непригоден,
                              кнопки b0..b23 (SA = b1, посадка); жесты арм/дизарм,
                              эквиваленты с хоста (make sa-land/rth/smart-rth), js_probe
...
LV2_loiter.md               — то же для LV=2 «GPS отсутствует с бута» (штатный
...
laptop_move.md              — переезд стенда на GPU-ноут + ЖИВОЙ ПУЛЬТ: петля
                              ...
                              §5 — план переезда на реальный борт (RX в FCU + отвод
...
переезда на реальный борт**: RX штатно в FCU + пассивный отвод CRSF-линии на Orin,
...
на fps; `sim_up.sh` патчит SDF. Реальный Orin и GPU-sim остаются на базовом compose.
...
ни при чём — лечится только референсом скорости/позиции (на реальном борту — VINS).
...
На реальном Orin костыля НЕТ — там референс даёт реальный VINS.
