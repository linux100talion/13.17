...
# Расчёт «сила = mass·factor·(v_ветра − v_звена)» даёт впятеро больше, чем плагин
# прикладывает на самом деле: прогон W0_windprobe при factor=0.15 и ветре 3 м/с показал
# 0.13 Н (оракул держал позицию креном 0.43°, ровно с первой секунды — нарастания нет),
# тогда как формула обещала 0.68 Н. Поэтому масштаб снят эмпирически: 0.87 Н на единицу
# factor при 3 м/с.
# Цель — реалистичное лобовое: ½ρv²·Cd·A = 0.5·1.225·9·1.0·0.1 ≈ 0.55-0.8 Н на 3 м/с,
# то есть 0.31-0.45 м/с² при массе 1.75 кг и крен удержания 1.8-2.6°.
# Отсюда 0.8 (≈0.7 Н). Проверка любой правки — тем же прогоном: BS_STAB=GzPosHold,
# средний крен на висении × g × масса и есть приложенная сила.
WIND_SPD="${WIND_SPD:-0}"
if [ "$WIND_SPD" != "0" ]; then
    WIND_DIR_DEG="${WIND_DIR_DEG:-98}"; WIND_FACTOR="${WIND_FACTOR:-0.8}"
    mkdir -p "$PATCH"
